...
describe('inside the family of markdown2html components', () => {
  ...
  describe('emPatternP', () => {
    ...
    it('converts a plain em', () => {
      expect(emPatternPrun('*em thing*').value[0]).toEqual('<em>em thing</em>', '"*em thing*" parses to "<em>em thing</em>"');
  ...
  describe('richTextStringP', () => {
    ...
    it('converts a plain string/strong/em', () => {
      // TODO restore when dashes are allowed in rich text strings
    ...
    it('converts a string containing bolds and ems', () => {
      expect(richTextStringPrun('plain **text** string').value[0]).toEqual('plain <strong>text</strong> string', 'richTextStringP parses plain strings');
  ...
  describe('listItemP', () => {
    ...
    it('converts a rich-text bulletpoint item', () => {
      const conv = listItemPrun('- ciao, **mamma**; ero *italico*\\n');
      expect(conv.value[0]).toEqual('<li>ciao, <strong>mamma</strong>; ero <em>italico</em></li>', 'listItemP parses <li>s');
    });
  });
  ...
  describe('ulP', () => {
    ...
    it('converts several rich-text bulletpoint item', () => {
      ...
      expect(conv.value[0]).toEqual('<ul><li>ciao, <strong>mamma</strong>; ero <em>italico</em></li><li>ciao, <strong>babbo</strong>; ero <em>italico</em></li></ul>', 'ulP parses <ul>s');
    });
  });
  ...
  describe('markdown2html, the real thing,', () => {
    ...
    describe('checks first of all for signs of previous conversions or HTML tags', () => {
      it('and fails when trailing <span>s are found', () => {
      ...
      it('and fails when the string contains HTML', () => {
        const input1 = 'whatever <em>is</em> wrong';
        const input2 = 'whatever <strong>is</strong> wrong';
        const input3 = 'whatever <a href="pippo">is</a> wrong';
        const input4 = 'whatever <ul><li>ciao</li></ul> wrong';
        expect(markdown2html(input1).isSuccess).toBe(false, 'HTML shall not be converted - 1');
        expect(markdown2html(input2).isSuccess).toBe(false, 'HTML shall not be converted - 2');
        expect(markdown2html(input3).isSuccess).toBe(false, 'HTML shall not be converted - 3');
        expect(markdown2html(input4).isSuccess).toBe(false, 'HTML shall not be converted - 4');
      });
